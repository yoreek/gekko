...
let columnsDebounceTimer: number | null = null
const columnsDebounceMs = 250
...
function updateColumns(): void {
  ...
  if (width <= 0) {
    // Transient zero-width measurement (hidden container, screenshot/viewport emulation, mid-HMR
    // teardown). Acting on it would collapse the grid to one column and - in edit mode - persist
    // that destroyed layout via the grid's change event.
    return
  }
...
function observeGridHost(): void {
  ...
  resizeObserver = new ResizeObserver(() => {
    // Debounced: fullPage screenshots / viewport emulation / mid-layout reflows report transient
    // bogus widths for a frame or two. Acting on one collapses the grid to a single column and
    // (in edit mode) persists that destroyed layout. Only a width that survives the debounce is a
    // real resize.
    if (columnsDebounceTimer !== null) {
      window.clearTimeout(columnsDebounceTimer)
    }
    columnsDebounceTimer = window.setTimeout(() => {
      columnsDebounceTimer = null
      updateColumns()
    }, columnsDebounceMs)
  })
