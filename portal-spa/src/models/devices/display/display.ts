...
export abstract class BaseDisplay<TBitmapFormat extends RasterImageFormat> {
  ...
  bitmapByteLength(width: number, height: number): number {
    ...
    return rasterImageCodecRegistry.get(this.bitmapFormat).resolveByteLength(size.width, size.height)
  }

  supportsBitmapSize(width: number, height: number): boolean {
    return this.bitmapByteLength(width, height) <= this.maxBitmapBytes
  }
  ...
  protected resizeBitmapWidget<TWidget extends DisplayBitmapWidget>(
    ...
    if (!this.supportsBitmapSize(size.width, size.height)) {
      return widget
    }
  ...
  async importBitmapFromFile(
    ...
    if (!this.supportsBitmapSize(width, height)) {
      throw new Error(`Bitmap payload exceeds ${this.maxBitmapBytes} bytes`)
    }
  ...
  protected normalizeBitmapSize(width: number, height: number): { width: number; height: number } {
    const size = clampBitmapSize(width, height)
    return this.supportsBitmapSize(size.width, size.height) ? size : clampBitmapSize(this.defaultBitmapWidth, this.defaultBitmapHeight)
  }
