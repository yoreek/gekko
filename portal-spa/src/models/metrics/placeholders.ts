...
export type MetricPlaceholderFilter = TemplateFilter
...
export function metricPlaceholderResolverFromCatalog(
  catalog: readonly MetricPlaceholderDescriptor[] = [],
): Record<string, TemplateResolverEntry> {
  const resolver: Record<string, TemplateResolverEntry> = {}
  for (const descriptor of catalog) {
    if (!descriptor.available) {
      continue
    }
    resolver[metricPlaceholderResolverKey(descriptor)] = {
      preview: descriptor.preview ?? '',
      previewNumber: descriptor.previewNumber,
