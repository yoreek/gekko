...
function onGridChange(): void {
  if (!props.editable || !grid || applyingColumnChange) {
    return
  }
  ...
  emit(
    ...
    nodes
      ...
      .map(node => ({
        ...
        x: node.x ?? 0,
        y: node.y ?? 0,
        // Dashboard cards have a fixed one-column footprint. Only their
        // position and content-driven height are persisted.
        w: 1,
        h: node.h ?? 1,
      })),
  )
...
// A programmatic column change re-lays widgets out (GridStack cannot restore free-placement x
// positions when expanding back from fewer columns), so its change events must never be persisted
// as if the user had moved cards.
let applyingColumnChange = false

watch(
  () => props.columns,
  columns => {
    applyingColumnChange = true
    grid?.column(Math.max(1, columns))
    void nextTick(() => {
      applyingColumnChange = false
    })
  },
)
