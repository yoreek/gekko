  ...
  <DeviceWidgetBase v-if="dense" :device="device" :editable="editable" :subtitle="subtitle">
    ...
    <div class="d-flex flex-column ga-2">
      ...
      <div class="d-flex flex-column">
        <span class="text-body-2">{{ config.width }}×{{ config.height }} monochrome display</span>
  ...
  <v-card v-else variant="tonal">
    <v-card-text class="d-flex flex-column ga-3">
      ...
      <div class="d-flex flex-column ga-1">
        <span class="text-body-1">{{ config.width }}×{{ config.height }} monochrome display</span>
...
const config = computed(() => props.device.config as Ssd1306ConfigDraft)
