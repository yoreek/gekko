...
const inkOnColor = 'rgb(255 255 255)'
const inkOffColor = 'rgb(0 0 0)'
const inkColor = computed(() => (props.widget.styleFlags.inverted ? inkOffColor : inkOnColor))
...
const textDataUrl = computed(() => {
  ...
  drawClassicFontText(ctx, props.previewText ?? props.widget.text, {
    ...
    color: props.widget.styleFlags.inverted ? inkOffColor : inkOnColor,
    backgroundColor: props.widget.styleFlags.inverted ? inkOnColor : inkOffColor,
  })
...
const bitmapDataUrl = computed(() => {
  ...
  canvas.width = Math.max(1, Math.round(props.widget.width))
  canvas.height = Math.max(1, Math.round(props.widget.height))
  props.display.renderWidget(props.widget, canvas, props.widget.styleFlags.inverted)
  return canvas.toDataURL()
})
</script>
