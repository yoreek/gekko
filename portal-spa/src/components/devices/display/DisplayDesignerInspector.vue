...
import DisplayColorField from './DisplayColorField.vue'
...
function updateBitmapDimension(key: 'width' | 'height', value: string | number): void {
  ...
  if (!props.display.supportsBitmapSize(nextSize.width, nextSize.height)) {
    bitmapError.value = t('device.dialog.display.bitmapTooLarge', { max: props.display.maxBitmapBytes })
    return
  }
  bitmapError.value = ''
  const resized = props.display.resizeWidget(currentWidget, nextSize)
  emit('update-widget', {
    width: resized.width,
    height: resized.height,
    bitmapData: resized.bitmapData,
  } as Partial<DisplayWidget>)
}
