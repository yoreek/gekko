...
function addWidget(type: Ssd1306WidgetType): void {
  ...
  const nextWidget = defaultSsd1306Widget(type, activePageWidgets.value.length)
    ...
    ...(widget.type === 'bitmap'
      ? {
          bitmapData: typeof widget.bitmapData === 'string' && widget.bitmapData.length > 0
            ? widget.bitmapData
            : createDefaultSsd1306BitmapData(Math.max(1, Math.min(width, OLED_DISPLAY_BITMAP_DEFAULT_WIDTH)), Math.max(1, Math.min(boundedHeight, OLED_DISPLAY_BITMAP_DEFAULT_HEIGHT))),
        }
      : {}),
  } as Ssd1306Widget
}
