<template>
  <v-dialog
    ...
    :max-width="ui.moreInfoMaxWidth ?? 520"
...
const ui = computed(() => resolveDeviceUi(props.device?.record.typeName))

function goToEdit(): void {
  if (!props.device) {
    return
  }
  const routeName = ui.value.designerComponent ? 'device-design' : 'device-detail'
