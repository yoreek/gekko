<template>
  <div class="d-flex flex-column ga-4">
    <v-alert v-if="dependencyItems.length === 0" type="warning" variant="tonal">
      {{ t('device.dialog.aht10.noDependency') }}
    </v-alert>
    ...
    <v-alert type="info" variant="tonal">
      {{ t('device.dialog.aht10.addressHint') }}
    </v-alert>
...
import type { DeviceRecord, Aht10SensorOutputSnapshot, TemperatureUnit } from '@/api/contracts'
import { Aht10Device, type Aht10ConfigDraft } from '@/models/devices/aht10'
...
const emit = defineEmits<{
  'update:modelValue': [value: Aht10ConfigDraft]
}>()
...
const temperatureUnitItems = computed(() =>
  Aht10Device.temperatureUnitOptions.map(value => ({ title: t(`device.dialog.temperatureUnit.${value}`), value })),
)

const output = computed(() => (props.device?.runtime as { output?: Aht10SensorOutputSnapshot } | undefined)?.output)
...
const temperatureText = computed(() =>
  output.value?.temperature?.valid
    ? Aht10Device.formatTemperature(output.value.temperature)
    : t('device.dialog.temperatureUnavailableShort'),
)
const humidityText = computed(() =>
  output.value?.humidity?.valid ? Aht10Device.formatHumidity(output.value.humidity) : t('device.dialog.temperatureUnavailableShort'),
)
