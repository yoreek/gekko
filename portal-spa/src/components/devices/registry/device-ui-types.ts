 ...
 * once the underlying condition clears — they carry no read/unread state.
 */
export interface DeviceAlert {
  /** Stable per-device alert kind; `${deviceId}:${kind}` keys the alert across recomputes. */
  kind: string
  severity: DeviceAlertSeverity
  /** i18n key resolved by the alerts UI so messages follow locale switches. */
  messageKey: string
...
export interface DeviceUi {
  ...
  /** Optional wider More Info dialog for content such as multi-channel charts. */
  readonly moreInfoMaxWidth?: string | number
  /**
  ...
  /**
   * Extracts standing alerts from the device's live runtime. Called reactively
   * by the deviceAlerts store for every registry device; omit for types that
   * never raise alerts. Must be a pure read of the passed record.
   */
