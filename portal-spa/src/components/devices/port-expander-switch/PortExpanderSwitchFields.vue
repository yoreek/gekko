<template>
  <div class="d-flex flex-column ga-4">
    <v-alert v-if="dependencyItems.length === 0" type="warning" variant="tonal">
      {{ t('device.dialog.portExpanderSwitch.noDependency') }}
    </v-alert>
    ...
    <v-row>
      ...
      <v-col cols="12" sm="6">
        <v-text-field
          type="number"
          :label="t('device.fields.portExpanderChannel')"
...
import type { PortExpanderSwitchConfigDraft } from '@/models/devices/port-expander-switch'
...
const props = defineProps<{
  modelValue: PortExpanderSwitchConfigDraft
  device?: DeviceRecord
...
const emit = defineEmits<{
  'update:modelValue': [value: PortExpanderSwitchConfigDraft]
...
const { update } = useDraftModel<PortExpanderSwitchConfigDraft>(props, emit)
