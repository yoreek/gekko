...
// Derive the build date from the current commit (committer date), NOT wall-clock time. A wall-clock
// `new Date()` changes on every build, so the entry bundle's content hash changes every time, which
// renames assets/i-[hash].js and churns index.html - making data/*.gz differ from what's committed
// on every rebuild even when nothing changed. Tying it to HEAD makes the build reproducible: the
// same source at the same commit always produces byte-identical output.
...
const appBuildDate = gitCommitDate()
