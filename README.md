...
[![Latest tag](https://img.shields.io/github/v/tag/yoreek/gekko)](https://github.com/yoreek/gekko/tags)

ESP32 firmware (PlatformIO, C++) + web portal (Vue SPA in `portal-spa/`), served by the firmware itself from LittleFS.

Firmware and SPA are versioned from `git describe` at build time, not from a hand-maintained version file — there's no release tag yet, so the tag badge above will read "no tag" until the first one is pushed. The exact running version is always visible at runtime:
