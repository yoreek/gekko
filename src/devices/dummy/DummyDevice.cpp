...
DummyDevice::DummyDevice(const DeviceRegistryEntry& record, const DeviceConfigBlob& configBlob)
    ...
    (void)decodeValidatedFixedConfigBlob(DeviceBaseConfigV1::kMagic, configBlob.data(), configBlob.size(), config_);
}
...
const DeviceBaseConfigV1& DummyDevice::baseConfig() const {
    return config_;
}
...
bool DummyDevice::serializeConfigBlob(DeviceConfigBlob& configBlob) const {
    ...
    return encodeFixedConfigBlob(DeviceBaseConfigV1::kMagic, config_, buffer, size) && configBlob.assign(buffer, size);
}

DeviceConfigUpdatePlan DummyDevice::planConfigUpdate(const DeviceConfigBlob& configBlob) const {
    DummyDeviceConfigV1 config{};
    if (!decodeValidatedFixedConfigBlob(DeviceBaseConfigV1::kMagic, configBlob.data(), configBlob.size(), config)) {
        return {};
    }
    return {};
}
