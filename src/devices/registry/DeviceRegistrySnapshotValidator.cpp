...
DeviceValidationResult DeviceRegistrySnapshotValidator::validateTypedRelationships(const DeviceRegistrySnapshot& snapshot,
    ...
    for (const auto& record : snapshot.records) {
        ...
        for (uint8_t index = 0; index < depCount && links != nullptr; ++index) {
            ...
            // MetricSource links are derived from display layout references and may point at any
            // device type; no descriptor declares MetricSource in providedRoles.
            if (requirementIt->role != DeviceRole::MetricSource && !dependencyDescriptor->providedRoles.contains(requirementIt->role)) {
            ...
            if (dependentDescriptor->exclusiveDependencyRoles.contains(link.role)) {
                const auto key = std::make_pair(dependencyRecord.header.deviceId, link.role);
                const auto owner = exclusiveOwners.find(key);
                if (owner != exclusiveOwners.end() && owner->second != record.header.deviceId) {
                    return {DeviceError::InvalidRelationship, "dependency already has a controlling owner"};
                }
