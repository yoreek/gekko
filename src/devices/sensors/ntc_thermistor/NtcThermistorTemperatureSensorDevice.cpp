...
NtcThermistorTemperatureSensorDevice::NtcThermistorTemperatureSensorDevice(const DeviceRegistryEntry& record,
    ...
    : NtcThermistorTemperatureSensorDevice([&configBlob]() {
          ...
          (void)decodeNtcThermistorTemperatureSensorConfig(configBlob.data(), configBlob.size(), config);
          return config;
      }()) {
...
bool NtcThermistorTemperatureSensorDevice::enabled() const {
    return config_.enabled != 0U;
}

const char* NtcThermistorTemperatureSensorDevice::name() const {
    return config_.name;
}
...
bool NtcThermistorTemperatureSensorDevice::serializeConfigBlob(DeviceConfigBlob& configBlob) const {
    ...
    return encodeNtcThermistorTemperatureSensorConfig(config_, buffer, size) && configBlob.assign(buffer, size);
}

bool NtcThermistorTemperatureSensorDevice::replaceBaseConfig(DeviceConfigBlob& configBlob, const DeviceBaseConfigV1& baseConfig) const {
    NtcThermistorTemperatureSensorConfigV1 config = config_;
    ...
    return encodeNtcThermistorTemperatureSensorConfig(config, buffer, size) && configBlob.assign(buffer, size);
}

DeviceConfigUpdatePlan NtcThermistorTemperatureSensorDevice::planConfigUpdate(const DeviceConfigBlob& configBlob) const {
    NtcThermistorTemperatureSensorConfigV1 config{};
    if (!decodeNtcThermistorTemperatureSensorConfig(configBlob.data(), configBlob.size(), config)) {
        return {};
    }
...
void NtcThermistorTemperatureSensorDevice::writeDeviceJson(JsonObject output) const {
