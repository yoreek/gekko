...
DeviceValidationResult validateDht11CommonConfig(const Dht11SensorConfigV3& config) {
...
bool parseDht11FieldsJson(const JsonObjectConst& input, Dht11SensorConfigV3& config, const char*& error) {
    ...
    const char* captureMode = input["captureMode"] | (config.captureMode == static_cast<uint8_t>(Dht11CaptureMode::Rmt) ? "rmt" : "native");
    if (std::strcmp(captureMode, "native") == 0) {
        config.captureMode = static_cast<uint8_t>(Dht11CaptureMode::Native);
    } else if (std::strcmp(captureMode, "rmt") == 0) {
        config.captureMode = static_cast<uint8_t>(Dht11CaptureMode::Rmt);
    } else {
        error = "dht11 capture mode is invalid";
        return false;
    }
...
void writeDht11FieldsJson(const Dht11SensorConfigV3& config, JsonObject output) {
    output["gpioPin"] = config.gpioPin;
    output["internalPullup"] = config.internalPullup != 0U;
    output["captureMode"] = config.captureMode == static_cast<uint8_t>(Dht11CaptureMode::Rmt) ? "rmt" : "native";
...
EWFM_LEGACY_CONFIG_USE_END
static_assert(std::is_trivially_copyable<Dht11SensorConfigV3>::value, "Dht11SensorConfigV3 must be POD");
static_assert(sizeof(Dht11SensorConfigV3::kMagic) - 1U + sizeof(Dht11SensorConfigV3) <= kMaxDeviceConfigBytes,
              "Dht11SensorConfigV3 exceeds device config bound");

bool decodeDht11SensorConfig(const uint8_t* blob, size_t size, Dht11SensorConfigV3& config) {
    if (decodeFixedConfigBlob(Dht11SensorConfigV3::kMagic, blob, size, config) && config.validate().ok()) {
        return true;
    }
    EWFM_LEGACY_CONFIG_USE_BEGIN
    Dht11SensorConfigV2 v2{};
    if (decodeFixedConfigBlob(Dht11SensorConfigV2::kMagic, blob, size, v2) && v2.validate().ok()) {
    ...
    Dht11SensorConfigV2 migrated{};
    migrated.migrateFrom(legacy);
    config.migrateFrom(migrated);
    EWFM_LEGACY_CONFIG_USE_END
    return true;
}
...
DeviceValidationResult Dht11SensorConfigV3::validate() const {
    const DeviceValidationResult baseValidation = DeviceBaseConfigV1::validate();
    if (!baseValidation.ok()) {
        return baseValidation;
    }
    if (!gpioSwitchPinIsValid(gpioPin) || internalPullup > 1U || captureMode > static_cast<uint8_t>(Dht11CaptureMode::Rmt)) {
        return {DeviceError::InvalidConfig, "dht11 gpio config is invalid"};
    }
    if (captureMode == static_cast<uint8_t>(Dht11CaptureMode::Rmt) && !RmtPulseCapture::supported()) {
        return {DeviceError::InvalidConfig, "dht11 rmt is unsupported on this target"};
    }
    return validateDht11CommonConfig(*this);
}

bool Dht11SensorConfigV3::parseJson(const JsonObjectConst& input, const char*& error) {
...
void Dht11SensorConfigV3::writeJson(JsonObject output) const {
    DeviceBaseConfigV1::writeJson(output);
    writeDht11FieldsJson(*this, output);
}

EWFM_LEGACY_CONFIG_USE_BEGIN
void Dht11SensorConfigV3::migrateFrom(const Dht11SensorConfigV2& legacy) {
    static_cast<DeviceBaseConfigV1&>(*this) = static_cast<const DeviceBaseConfigV1&>(legacy);
    gpioPin = legacy.gpioPin;
    internalPullup = legacy.internalPullup;
    outputUnit = legacy.outputUnit;
    reportAlways = legacy.reportAlways;
    reportDeltaCentiCelsius = legacy.reportDeltaCentiCelsius;
    reportDeltaCentiPercent = legacy.reportDeltaCentiPercent;
    ...
    temperatureFilter = legacy.temperatureFilter;
    humidityFilter = legacy.humidityFilter;
    captureMode = static_cast<uint8_t>(Dht11CaptureMode::Native);
}
EWFM_LEGACY_CONFIG_USE_END
