...
struct ParsedPlaceholder {
    ...
    char filterArg[kDisplayTextFilterArgCapacity]{};
};
    ...
    bool get(const MetricValueResolver& resolver, MetricValue& value) const override {
        ...
        case MetricNamespace::Device:
            ...
            case kDeviceMetricTemperature: {
                ...
                std::snprintf(value.text, sizeof(value.text), "%.2f C", static_cast<double>(celsius));
                return true;
            }
            case kDeviceMetricHumidity: {
                const IHumidityReadingRuntime* humidity = runtime_->humidityReadingRuntime();
                if (humidity == nullptr) {
                    return false;
                }
                ...
                std::snprintf(value.text, sizeof(value.text), "%.1f %%", static_cast<double>(percent));
                return true;
            }
            case kDeviceMetricSwitchState: {
                ...
                std::snprintf(value.text, sizeof(value.text), "%s", switchOutputStateName(switchOutput->currentOutputState()));
...
bool metricKeyToId(const MetricNamespace ns, const char* key, const size_t keyLength, int32_t& metricId) {
    ...
    case MetricNamespace::Device:
        ...
        if (equals("humidity")) {
            metricId = kDeviceMetricHumidity;
            return true;
        }
        if (equals("state")) {
...
// Parses a "0".."6" decimal digit count (the `fixed:<digits>` filter argument). Unlike
// parseUnsigned() below, 0 is a valid value here (fixed:0 means "no decimal places").
bool parseFixedDigits(const char* begin, const char* end, uint8_t& digits) {
    if (begin == nullptr || end == nullptr || begin >= end) {
        return false;
    }
    uint32_t value = 0U;
    ...
    digits = static_cast<uint8_t>(value);
    return true;
}

// Trims [begin,end) and copies the result into argOut (capacity-checked, null-terminated).
// Fails if the trimmed span is empty or doesn't fit.
bool copyTrimmedFilterArg(const char* begin, const char* end, char* argOut, const size_t argCapacity) {
    ...
    if (length == 0U || length >= argCapacity) {
        return false;
    }
    std::memcpy(argOut, begin, length);
    argOut[length] = '\0';
    return true;
}

// Parses "name" or "name:arg" (the part after "|"). text/upper/lower/trim never take an argument;
// format requires a non-empty pattern argument; fixed requires an argument parsing to 0-6.
bool parseFilter(const char* begin, const char* end, DisplayTextFilterKind& filter, char* argOut, const size_t argCapacity) {
    while (begin < end && std::isspace(static_cast<unsigned char>(*begin))) {
        ++begin;
    }
    while (end > begin && std::isspace(static_cast<unsigned char>(*(end - 1)))) {
    ...
    if (argOut != nullptr && argCapacity > 0U) {
        argOut[0] = '\0';
    }
    if (begin >= end) {
    ...
    const char* colon = static_cast<const char*>(std::memchr(begin, ':', static_cast<size_t>(end - begin)));
    const char* nameEnd = colon != nullptr ? colon : end;
    const char* argBegin = colon != nullptr ? colon + 1 : end;

    const size_t nameLength = static_cast<size_t>(nameEnd - begin);
    const auto nameEquals = [&](const char* expected) {
        return std::strlen(expected) == nameLength && std::strncmp(begin, expected, nameLength) == 0;
    };

    if (nameEquals("text") || nameEquals("upper") || nameEquals("lower") || nameEquals("trim")) {
        if (colon != nullptr) {
            return false; // these filters never take an argument
        }
        filter = nameEquals("text")    ? DisplayTextFilterKind::Text
                 : nameEquals("upper") ? DisplayTextFilterKind::Upper
                 : nameEquals("lower") ? DisplayTextFilterKind::Lower
                                       : DisplayTextFilterKind::Trim;
        return true;
    }
    if (nameEquals("format")) {
        if (colon == nullptr || !copyTrimmedFilterArg(argBegin, end, argOut, argCapacity)) {
            return false;
        }
        filter = DisplayTextFilterKind::Format;
        return true;
    }
    if (nameEquals("fixed")) {
        if (colon == nullptr || !copyTrimmedFilterArg(argBegin, end, argOut, argCapacity)) {
        ...
        uint8_t digits = 0U;
        if (!parseFixedDigits(argOut, argOut + std::strlen(argOut), digits)) {
            return false;
        }
        filter = DisplayTextFilterKind::Fixed;
        return true;
    }
    return false;
}
...
bool parsePlaceholderBody(const char* begin, const char* end, ParsedPlaceholder& parsed, DisplayTextCompileStatus& status) {
    ...
    DisplayTextFilterKind filter = DisplayTextFilterKind::None;
    char filterArg[kDisplayTextFilterArgCapacity]{};
    if (!parseFilter(filterBegin, end, filter, filterArg, sizeof(filterArg))) {
        status = DisplayTextCompileStatus::MalformedSyntax;
        return false;
    }
    parsed.ns = ns;
    ...
    std::snprintf(parsed.filterArg, sizeof(parsed.filterArg), "%s", filterArg);
    return true;
}
...
// Text/Upper/Lower/Trim seed `output` from the resolver's preformatted value.text and transform
// it in place. Format/Fixed need the raw typed MetricValue instead - a preformatted string has no
// calendar timestamp or numeric precision left to reformat.
bool applyDisplayTextFilter(const MetricValue& value, const DisplayTextFilterKind filter, const char* filterArg, char* output,
    ...
    case DisplayTextFilterKind::Text:
        std::snprintf(output, capacity, "%s", value.text);
        return true;
    case DisplayTextFilterKind::Upper:
        std::snprintf(output, capacity, "%s", value.text);
        for (size_t index = 0; output[index] != '\0'; ++index) {
            output[index] = static_cast<char>(std::toupper(static_cast<unsigned char>(output[index])));
        }
        return true;
    case DisplayTextFilterKind::Lower:
        std::snprintf(output, capacity, "%s", value.text);
        for (size_t index = 0; output[index] != '\0'; ++index) {
            output[index] = static_cast<char>(std::tolower(static_cast<unsigned char>(output[index])));
        }
        return true;
    case DisplayTextFilterKind::Trim: {
        std::snprintf(output, capacity, "%s", value.text);
        const size_t total = std::strlen(output);
        size_t begin = 0U;
        size_t end = total;
        while (begin < end && std::isspace(static_cast<unsigned char>(output[begin]))) {
            ++begin;
        }
        while (end > begin && std::isspace(static_cast<unsigned char>(output[end - 1U]))) {
            --end;
        }
        if (begin > 0U) {
            std::memmove(output, output + begin, end - begin);
        }
        output[end - begin] = '\0';
        return true;
    }
    case DisplayTextFilterKind::Format:
        if (value.valueType == MetricValueType::DateTime) {
            const auto* dateTimeValue = std::get_if<DateTime>(&value.number);
            return dateTimeValue != nullptr && formatDateTimePattern(*dateTimeValue, filterArg, output, capacity);
        }
        if (value.valueType == MetricValueType::Duration) {
            const auto* durationValue = std::get_if<int32_t>(&value.number);
            return durationValue != nullptr && formatDurationPattern(static_cast<uint32_t>(*durationValue), filterArg, output, capacity);
        }
        return false;
    case DisplayTextFilterKind::Fixed: {
        uint8_t digits = 0U;
        if (!parseFixedDigits(filterArg, filterArg + std::strlen(filterArg), digits)) {
            return false;
        }
        return formatFixedDecimals(value, digits, output, capacity);
    }
    }
    return false;
}
...
void appendResolvedText(std::string_view sourceText, const DisplayTextCompiledWidget& compiled, const MetricValueResolver& resolver,
    ...
    for (const DisplayTextSegment& segment : compiled.segments) {
        ...
        if (resolved) {
            ...
            if (applyDisplayTextFilter(value, segment.filter, segment.filterArg, filtered, sizeof(filtered))) {
...
DisplayTextCompileResult compileDisplayTextWidget(std::string_view text) {
    ...
    while (cursor < end) {
        ...
        std::snprintf(segment.filterArg, sizeof(segment.filterArg), "%s", parsed.filterArg);
        result.compiled.segments.push_back(segment);
        cursor = placeholderEnd + 2;
    }
...
bool displayTextPlaceholderSupportsRuntime(const IDeviceRuntime& runtime, const DisplayTextPlaceholderSegment& placeholder) {
    ...
    case kDeviceMetricHumidity:
        return runtime.humidityReadingRuntime() != nullptr;
    case kDeviceMetricSwitchState:
