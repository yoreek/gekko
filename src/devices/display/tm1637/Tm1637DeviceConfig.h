...
// survive) but reports its pins as unset (kGpioPinUnset, DeviceTypes.h); the runtime then faults
// on start until the pins are filled in from the portal. REST never accepts an unset pin - see
// Tm1637DeviceConfigV2::parseJson.
...
struct Tm1637DeviceConfigV2 : DeviceBaseConfigV1 {
    ...
    uint8_t brightness{7U};
    uint8_t rotation{0U};
    uint8_t clkPin{kGpioPinUnset};
    uint8_t dioPin{kGpioPinUnset};
