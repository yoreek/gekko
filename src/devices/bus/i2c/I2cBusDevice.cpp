...
I2cBusDevice::I2cBusDevice(const DeviceRegistryEntry& record, const DeviceConfigBlob& configBlob)
    : I2cBusDevice(
          [&configBlob]() {
              ...
              (void)decodeValidatedFixedConfigBlob(I2cBusDeviceConfigV1::kMagic, configBlob.data(), configBlob.size(), config);
              return config;
          }(),
...
const DeviceBaseConfigV1& I2cBusDevice::baseConfig() const {
    return config_;
}
...
bool I2cBusDevice::serializeConfigBlob(DeviceConfigBlob& configBlob) const {
    ...
    return encodeFixedConfigBlob(I2cBusDeviceConfigV1::kMagic, config_, buffer, size) && configBlob.assign(buffer, size);
}

DeviceConfigUpdatePlan I2cBusDevice::planConfigUpdate(const DeviceConfigBlob& configBlob) const {
    I2cBusDeviceConfigV1 config{};
    if (!decodeValidatedFixedConfigBlob(I2cBusDeviceConfigV1::kMagic, configBlob.data(), configBlob.size(), config)) {
        return {};
    }
...
DeviceTypeDescriptor I2cBusDevice::descriptor() {
    ...
    descriptor.providedRoles = ProvidedRoles::of({II2cBusRuntime::kProvidedRole});
