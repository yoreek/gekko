...
OneWireBusDevice::OneWireBusDevice(const DeviceRegistryEntry& record, const DeviceConfigBlob& configBlob)
    : OneWireBusDevice(
          [&configBlob]() {
              ...
              (void)decodeValidatedFixedConfigBlob(OneWireBusDeviceConfigV1::kMagic, configBlob.data(), configBlob.size(), config);
              return config;
          }(),
...
const DeviceBaseConfigV1& OneWireBusDevice::baseConfig() const {
    return config_;
}
...
bool OneWireBusDevice::serializeConfigBlob(DeviceConfigBlob& configBlob) const {
    ...
    return encodeFixedConfigBlob(OneWireBusDeviceConfigV1::kMagic, config_, buffer, size) && configBlob.assign(buffer, size);
}

DeviceConfigUpdatePlan OneWireBusDevice::planConfigUpdate(const DeviceConfigBlob& configBlob) const {
    OneWireBusDeviceConfigV1 config{};
    if (!decodeValidatedFixedConfigBlob(OneWireBusDeviceConfigV1::kMagic, configBlob.data(), configBlob.size(), config)) {
        return {};
    }
...
DeviceTypeDescriptor OneWireBusDevice::descriptor() {
    ...
    descriptor.providedRoles = ProvidedRoles::of({IOneWireBusRuntime::kProvidedRole});
