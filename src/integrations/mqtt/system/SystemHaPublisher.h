...
// firmware version, firmware build date) and a restart button to Home Assistant, under the same HA device as the per-DeviceRegistry-device
...
private:
    ...
    std::string topicFor(const char* key, const char* suffix, bool discoveryPayload = false) const;
