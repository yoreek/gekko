...
public:
    ...
    static std::string buildTimeStatus(bool synced, uint32_t currentEpochUtc, const char* timezoneId, const char* source,
                                       uint32_t revision);

private:
    static std::string buildEnvelope(const char* topic, uint32_t revision, const JsonDocument& payload);
