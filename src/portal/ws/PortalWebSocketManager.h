...
public:
    ...
    void tick(uint32_t now, const WifiManager& wifiManager, const IWifiDriver& wifiDriver, bool otaEnabled, bool otaHasError,
              uint32_t freeSketchSpace, bool mqttEnabled, bool mqttConnected, bool mqttWaitingForStation, const NtpManager* ntpManager);
    ...
    void publishSnapshotPayloadsForTest(const std::string& wifiPayload, const std::string& otaPayload,
                                        const std::string& mqttPayload = std::string(), const std::string& timePayload = std::string());
#endif
...
private:
    ...
    std::string lastTimeStatusPayload_{};
