...
bool BaseController::ChunkedBody::emit(const char* data, size_t n) {
    if (!buf.ensure(n)) {
        len = 0;
        pos = 0;
        return false;
...
bool BaseController::ChunkedBody::emitJson(JsonDocument& doc, bool leadingComma) {
    const size_t jsonLen = measureJson(doc);
    const size_t offset = leadingComma ? 1U : 0U;
    // measureJson excludes the terminating NUL that serializeJson writes, hence the +1.
    if (!buf.ensure(jsonLen + offset + 1U)) {
        len = 0;
        pos = 0;
        return false;
    }
    ...
    serializeJson(doc, buf.data() + offset, jsonLen + 1U);
    len = jsonLen + offset;
    pos = 0;
    return true;
}

void BaseController::sendChunked(const char* contentType, ChunkedProducer producer,
                                 const std::function<void(AsyncWebServerResponse*)>& configure) {
#if defined(ARDUINO) && !defined(UNIT_TEST)
    if (request_ == nullptr) {
        return;
    }
    // Producer and body live on the async_tcp task, which runs the filler after this controller is
    // long gone - hold both in a shared_ptr captured by the filler lambda.
    struct StreamState {
        ChunkedBody body;
        ChunkedProducer produce;
    };
    auto state = std::make_shared<StreamState>();
    state->produce = std::move(producer);
    AsyncWebServerResponse* response =
        request_->beginChunkedResponse(contentType, [state](uint8_t* out, size_t maxLen, size_t index) -> size_t {
            (void)index;
            ChunkedBody& body = state->body;
            size_t written = 0;
            while (written < maxLen) {
                if (body.pos < body.len) {
                    const size_t n = std::min(maxLen - written, body.len - body.pos);
                    std::memcpy(out + written, body.buf.data() + body.pos, n);
                    body.pos += n;
                    written += n;
                    continue;
                }
