...
- **THEN** the SPA sends one `update_config` command to `POST /api/devices/:id/command` that can carry DS18B20 config and parent fields together as structured JSON, and the API validates the combined mutation before applying it
...
- **THEN** the API receives `command = "scan"` for the selected OneWire parent device without a public payload string
