...
- **THEN** `config.startupState` and `config.safeState` remain persisted configuration settings independent from `runtime.output.state`
...
- **THEN** the request uses `command = "setStatus"` with a string `status` field and does not use `payload` for the status value

#### Scenario: Set deps remains structured
- **WHEN** a client changes a device dependency relationship
- **THEN** the request uses `command = "updateConfig"` with a structured `deps` array when dependency changes are applied with config, rather than dependency fields or a packed payload string
...
- **WHEN** a client sends `command = "updateConfig"` for a DS18B20 temperature sensor
...
- **WHEN** a client sends `command = "updateConfig"` for a OneWire bus
- **THEN** the OneWire API adapter requires a JSON `config` object containing `enabled`, `gpioPin`, and `internalPullup`, parses and validates it, encodes the internal config payload, and rejects requests that provide only binary `payload`

#### Scenario: GPIO switch update accepts only JSON config
- **WHEN** a client sends `command = "updateConfig"` for a GPIO switch
...
The portal API SHALL expose device dependency links as `config.deps` in canonical device snapshots.

#### Scenario: Snapshot includes deps
- **WHEN** the API serializes a device with dependency links
- **THEN** the snapshot includes `config.deps` entries with `role` and `deviceId`
...
- **THEN** the error response reports `dependentDeviceIds`
...
- **THEN** the request uses `command = "setOutput"` with `state = "on"`, `"off"`, or `"disabled"` and does not use a packed `state=...` payload string
...
- **THEN** the request includes `typeName = "ds18b20_temperature_sensor"` and a `config` object containing common device fields, a `deps` entry with role `onewire_bus`, address, resolution, unit, poll period, report delta, and report policy
...
- **THEN** the SPA sends one `updateConfig` command to `POST /api/devices/:id/command` that can carry DS18B20 config and dependency fields together as structured JSON, and the API validates the combined mutation before applying it
...
- **THEN** the request includes `typeName = "thermostat"` and a `config` object containing common device fields, `temperature_sensor` and `switch` deps, mode, target, hysteresis, safe min/max, check interval, sensor timeout, retry timeout, and minimum switch interval
...
- **THEN** the SPA sends one `updateConfig` command to `POST /api/devices/:id/command` that can carry thermostat config and dependency fields together as structured JSON, and the API validates the combined mutation before applying it

#### Scenario: Config-only update accepts unchanged deps
- **WHEN** a thermostat `updateConfig` request includes `deps` entries whose `temperature_sensor` and `switch` device ids match the current dependency links and changes only thermostat control config fields
...
- **WHEN** a thermostat `updateConfig` request changes the `temperature_sensor` or `switch` dependency device id
