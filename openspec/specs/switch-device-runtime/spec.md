# switch-device-runtime Specification

## Purpose

Define shared switch-device runtime behavior and the GPIO switch implementation.
...
- **WHEN** an accepted GPIO switch config update changes `gpioPin`
...
- **WHEN** a GPIO switch state machine restarts after a `gpioPin` or `inverted` config update
...
- **WHEN** an accepted GPIO switch config update changes only `startupState`, `safeState`, or `restorePreviousState`
...
- **THEN** the firmware rejects the config before calling `end`, changing runtime config, resetting the state machine, or changing persistence state
