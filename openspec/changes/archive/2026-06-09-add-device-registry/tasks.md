...
- [x] 7.3 Add minimal Web UI controls for `DummyDevice` create/list/edit/delete/enable/disable/status-command flows using device IDs as stable references.
...
- [x] 7.5 Expose retained-state-dependent startup behavior in API responses without embedding frequently changing retained state in the configuration payload.
...
- [x] 7.7 Add host-testable route or JSON codec coverage for successful and rejected device API payloads, version/revision fields, pending persistence state, explicit save/apply behavior, and retained-state restore metadata.
