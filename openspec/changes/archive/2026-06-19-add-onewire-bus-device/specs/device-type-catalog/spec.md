...
#### Scenario: Catalog contains the OneWire bus type
- **WHEN** the frontend renders the device type catalog after OneWire bus support is added
- **THEN** it exposes `OneWireBusDevice` with a stable numeric `type_id = 3` as a supported catalog entry

#### Scenario: OneWire bus metadata is available
- **WHEN** the frontend builds type-specific forms, detail sections, scan controls, or widgets for `OneWireBusDevice`
- **THEN** the catalog exposes the label key, local icon key, and component registry key required to resolve OneWire bus UI components without installing icon packages
...
- **THEN** it displays the localized label for `DummyDevice`, `GpioSwitchDevice`, or `OneWireBusDevice` while preserving the numeric `type_id` in data
