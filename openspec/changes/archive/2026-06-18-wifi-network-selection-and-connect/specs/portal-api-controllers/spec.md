...
#### Scenario: WiFi configure endpoint supports credential updates
- **WHEN** a client sends `POST /api/wifi/configure`
- **THEN** the migrated controller validates the SSID and password, stores the credentials through the WiFi configuration flow, and attempts to connect as a station

#### Scenario: WiFi configure endpoint supports credential reset
- **WHEN** a client sends `DELETE /api/wifi/configure`
- **THEN** the migrated controller clears saved WiFi credentials through the WiFi manager and returns a success envelope suitable for AP recovery

#### Scenario: BLE config endpoint remains available
- **WHEN** a client sends `POST /api/wifi/ble-config`
- **THEN** the migrated controller requests BLE provisioning mode through the WiFi manager and returns the existing accepted response shape
