...
#### Scenario: Device dashboard is primary workspace
- **WHEN** the portal renders the main application shell
- **THEN** the dashboard centers device cards and a device detail modal as the primary operational workflow
...
#### Scenario: Dashboard assets remain within budget
- **WHEN** device dashboard UI components are added or changed
- **THEN** the build continues to honor the `220 KB` LittleFS budget and avoids bringing in external icon or form libraries
...
#### Scenario: Dashboard actions are smoke tested
- **WHEN** Playwright exercises the device dashboard modal in mocked mode
- **THEN** the UI can open a device card, inspect details, and trigger supported mock actions without breaking the page
