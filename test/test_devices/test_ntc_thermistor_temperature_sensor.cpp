...
BoundedBlob<kMaxDeviceConfigBytes> encodeNtcPayload(const NtcThermistorTemperatureSensorConfigV1& config) {
    ...
    TEST_ASSERT_TRUE(encodeNtcThermistorTemperatureSensorConfig(config, buffer, ntcThermistorTemperatureSensorConfigSize(config)));
...
void test_ntc_thermistor_config_codec_json_and_validation() {
    ...
    TEST_ASSERT_TRUE(decodeNtcThermistorTemperatureSensorConfig(reinterpret_cast<const uint8_t*>(blob.data()), blob.size(), decoded));
