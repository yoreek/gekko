...
std::unique_ptr<IDeviceRuntime> createRuntime(const DeviceRegistryEntry& record, const DeviceConfigBlob& configBlob) {
    ...
    TEST_ASSERT_TRUE(decodeValidatedFixedConfigBlob(OneWireBusDeviceConfigV1::kMagic, reinterpret_cast<const uint8_t*>(configBlob.data()),
...
DeviceCreateRequest makeCreateRequest(uint8_t pin) {
    ...
    TEST_ASSERT_TRUE(request.assignName("onewire"));
...
void test_onewire_registry_create_scan_reconfigure_disable_and_delete() {
    ...
    OneWireBusDeviceConfigV1 disabledConfig{};
    TEST_ASSERT_TRUE(
        decodeValidatedFixedConfigBlob(OneWireBusDeviceConfigV1::kMagic, currentBlob.data(), currentBlob.size(), disabledConfig));
    disabledConfig.enabled = 0U;
    currentBlob = encodeOneWirePayload(disabledConfig);
    DeviceMutationResult disabled =
        registry.updateConfigAndDeps(created.deviceId, currentBlob, OneWireBusDevice::descriptor().currentConfigVersion, disabledConfig,
                                     false, {}, 0, 30, DevicePersistencePolicy::Delayed);
