...
void test_metric_value_resolver_resolves_device_temperature_and_missing_sources() {
    ...
    // Humidity is gated on humidityReadingRuntime() the same way temperature is on its runtime.
    TEST_ASSERT_FALSE(resolver.resolve(MetricNamespace::Device, dummyId, kDeviceMetricHumidity, value));
    TEST_ASSERT_FALSE(resolver.resolve(MetricNamespace::Device, 999, kDeviceMetricHumidity, value));
}
...
void test_metric_value_resolver_resolves_system_values() {
    ...
    // No localTime passed -> default-constructed DateTime() (year 1970) reads as "wall clock not
    // set yet", same as any other missing metric.
    TEST_ASSERT_FALSE(resolver.resolve(MetricNamespace::System, 0, kSystemMetricTime, value));
    TEST_ASSERT_EQUAL(MetricValueType::Null, value.valueType);

    const DateTime fixedTime(2026, 7, 11, 20, 15, 0);
    MetricValueResolver resolverWithTime(nullptr, wifi, 3723000U, fixedTime);
    TEST_ASSERT_TRUE(resolverWithTime.resolve(MetricNamespace::System, 0, kSystemMetricTime, value));
    TEST_ASSERT_EQUAL(MetricValueType::DateTime, value.valueType);
    TEST_ASSERT_TRUE(std::get<DateTime>(value.number) == fixedTime);
    TEST_ASSERT_EQUAL_STRING("20:15:00", value.text);

    TEST_ASSERT_TRUE(resolver.resolve(MetricNamespace::System, 0, kSystemMetricUptime, value));
    TEST_ASSERT_EQUAL(MetricValueType::Duration, value.valueType);
    TEST_ASSERT_EQUAL_INT32(3723000, std::get<int32_t>(value.number));
...
void test_display_text_evaluator_resolves_static_and_placeholder_text() {
    ...
    const DateTime fixedTime(2026, 7, 11, 20, 15, 0);
    MetricValueResolver timeResolver(nullptr, wifi, 3723000U, fixedTime);
    ...
    TEST_ASSERT_TRUE(result.available);
    TEST_ASSERT_EQUAL(DisplayTextEvaluationStatus::Resolved, result.status);
    TEST_ASSERT_EQUAL_STRING("Time 20:15:00", result.text);
...
void test_display_text_evaluator_handles_missing_and_binding_only_metrics() {
    ...
    TEST_ASSERT_TRUE(result.dynamic);
    TEST_ASSERT_TRUE(result.available);
    TEST_ASSERT_EQUAL_STRING("20:15:00", result.text);
...
void test_display_text_evaluator_applies_format_and_fixed_filters() {
    FakeWifiDriver wifi;
    const DateTime fixedTime(2026, 7, 11, 20, 15, 0); // Saturday
    MetricValueResolver resolver(nullptr, wifi, 3723000U, fixedTime);

    DisplayLayoutWidgetV1 widget{};
    widget.type = static_cast<uint8_t>(DisplayLayoutWidgetType::Text);
    DisplayTextEvaluationResult result{};

    std::snprintf(widget.text, sizeof(widget.text), "%s", "Day {{system.time | format:EEEE}}");
    TEST_ASSERT_TRUE(evaluateDisplayTextWidget(widget, resolver, result));
    TEST_ASSERT_EQUAL(DisplayTextEvaluationStatus::Resolved, result.status);
    TEST_ASSERT_EQUAL_STRING("Day Saturday", result.text);

    std::snprintf(widget.text, sizeof(widget.text), "%s", "Up {{system.uptime | format:HH:mm:ss}}");
    TEST_ASSERT_TRUE(evaluateDisplayTextWidget(widget, resolver, result));
    TEST_ASSERT_EQUAL(DisplayTextEvaluationStatus::Resolved, result.status);
    TEST_ASSERT_EQUAL_STRING("Up 01:02:03", result.text);

    std::snprintf(widget.text, sizeof(widget.text), "%s", "Up {{system.uptime | fixed:1}}");
    TEST_ASSERT_TRUE(evaluateDisplayTextWidget(widget, resolver, result));
    TEST_ASSERT_EQUAL(DisplayTextEvaluationStatus::Resolved, result.status);
    TEST_ASSERT_EQUAL_STRING("Up 3723000.0", result.text);

    // Format/Fixed applied to a value kind they don't support fail soft to empty, same as any
    // other missing metric - a bad placeholder does not cancel the whole widget's render.
...
void test_display_text_evaluator_rejects_malformed_filter_arguments() {
    ...
    std::snprintf(widget.text, sizeof(widget.text), "%s", "{{system.time | format}}");
    TEST_ASSERT_TRUE(evaluateDisplayTextWidget(widget, resolver, result));
    TEST_ASSERT_EQUAL(DisplayTextEvaluationStatus::InvalidPlaceholder, result.status);
    ...
    std::snprintf(widget.text, sizeof(widget.text), "%s", "{{system.uptime | fixed:9}}");
    TEST_ASSERT_TRUE(evaluateDisplayTextWidget(widget, resolver, result));
    TEST_ASSERT_EQUAL(DisplayTextEvaluationStatus::InvalidPlaceholder, result.status);

    std::snprintf(widget.text, sizeof(widget.text), "%s", "{{system.uptime | fixed:abc}}");
    TEST_ASSERT_TRUE(evaluateDisplayTextWidget(widget, resolver, result));
    TEST_ASSERT_EQUAL(DisplayTextEvaluationStatus::InvalidPlaceholder, result.status);

    std::snprintf(widget.text, sizeof(widget.text), "%s", "{{system.wifi.station_ip | upper:x}}");
    TEST_ASSERT_TRUE(evaluateDisplayTextWidget(widget, resolver, result));
    TEST_ASSERT_EQUAL(DisplayTextEvaluationStatus::InvalidPlaceholder, result.status);
}
