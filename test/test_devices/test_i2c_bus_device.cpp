...
void test_i2c_address_helpers_format_parse_and_validate() {
    ...
    json["i2cAddress"] = 0x7F;
    uint8_t jsonAddress = 0;
    const char* error = nullptr;
    TEST_ASSERT_TRUE(parseI2cAddressJson(json["i2cAddress"], jsonAddress, error));
    TEST_ASSERT_EQUAL_UINT8(0x7FU, jsonAddress);
    TEST_ASSERT_TRUE(validateI2cAddress(jsonAddress).ok());

    json["i2cAddress"] = -1;
    TEST_ASSERT_FALSE(parseI2cAddressJson(json["i2cAddress"], jsonAddress, error));
    TEST_ASSERT_EQUAL_STRING("i2c address is out of bounds", error);
    json["i2cAddress"] = "0x40";
    TEST_ASSERT_FALSE(parseI2cAddressJson(json["i2cAddress"], jsonAddress, error));
    TEST_ASSERT_EQUAL_STRING("i2c address must be numeric", error);
    const DeviceValidationResult validation = validateI2cAddress(0x80U);
    TEST_ASSERT_FALSE(validation.ok());
    TEST_ASSERT_EQUAL_STRING("i2c address is out of bounds", validation.message);

    writeI2cAddressJson(0x40U, json);
    TEST_ASSERT_EQUAL_UINT8(0x40U, json["i2cAddress"].as<uint8_t>());
}
...
void test_i2c_bus_config_codec_and_validation() {
    ...
    TEST_ASSERT_TRUE(decodeValidatedFixedConfigBlob(I2cBusDeviceConfigV1::kMagic, payload.data(), payload.size(), decoded));
