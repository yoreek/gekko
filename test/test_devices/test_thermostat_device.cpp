...
struct OutputWrite {
    ISwitchOutputRuntime::StateType state{false};
...
class FakeSwitch final : public SwitchDeviceBase {
public:
...
private:
    ...
    DeviceValidationResult applyHardwareOutput(bool physicalLevel, uint32_t now) override {
        const StateType state = physicalLevel != config_.inverted;
...
struct RegistrySwitchRuntime final : public SwitchDeviceBase {
    RegistrySwitchRuntime(const DeviceRegistryEntry& record, const DeviceConfigBlob& configBlob)
        : RegistrySwitchRuntime([&configBlob]() {
              ...
              (void)decodeValidatedFixedConfigBlob(SwitchDeviceConfigV2::kMagic, configBlob.data(), configBlob.size(), config);
              return config;
          }()) {
        bindDeviceIdentity(record, configBlob);
    }

    explicit RegistrySwitchRuntime(const SwitchDeviceConfigV2& config) : SwitchDeviceBase(config), config_(config) {}
...
SwitchDeviceConfigV2 makeSwitchConfig(ISwitchOutputRuntime::StateType startup = kSwitchOutputOff,
                                      ISwitchOutputRuntime::StateType safe = kSwitchOutputOff) {
    SwitchDeviceConfigV2 config{};
    config.enabled = true;
...
void startSwitch(FakeSwitch& device) {
    device.begin(10);
...
void test_thermostat_descriptor_accepts_any_standard_temperature_sensor_type() {
    ...
    TEST_ASSERT_TRUE(Ds18b20TemperatureSensorDevice::descriptor().providedRoles.contains(DeviceRole::TemperatureSensor));
    TEST_ASSERT_TRUE(NtcThermistorTemperatureSensorDevice::descriptor().providedRoles.contains(DeviceRole::TemperatureSensor));
}
...
void test_thermostat_config_codec_and_validation() {
    ...
    TEST_ASSERT_TRUE(decodeValidatedFixedConfigBlob(ThermostatDeviceConfigV1::kMagic, payload.data(), payload.size(), decoded));
    ...
    TEST_ASSERT_FALSE((decoded).validate().ok());
}
...
void test_runtime_capabilities_are_exposed() {
    ...
    FakeSwitch switchDevice(makeSwitchConfig());
    ...
    TEST_ASSERT_TRUE(switchDevice.switchOutputRuntime()->currentOutputState() == kSwitchOutputOff);
}
...
void test_registry_captures_retained_state_after_internal_switch_output_change() {
    ...
    SwitchDeviceConfigV2 config = makeSwitchConfig();
    DeviceCreateRequest request{};
    request.typeId = descriptor.typeId;
    TEST_ASSERT_TRUE(request.assignName("registry-switch"));
    ...
    TEST_ASSERT_TRUE(switchRuntime->requestOutputState(kSwitchOutputOn, 21));
...
void test_thermostat_heats_holds_and_cools() {
    ...
    TEST_ASSERT_TRUE(switchDevice.currentOutputState() == kSwitchOutputOff);

    sensor.setReading(24000, 110);
    thermostat.tick100ms(200);
    thermostat.tick100ms(201);
    thermostat.tick100ms(202);
    TEST_ASSERT_TRUE(switchDevice.currentOutputState() == kSwitchOutputOn);
    TEST_ASSERT_TRUE(thermostat.desiredOutputState() == kSwitchOutputOn);
...
void test_thermostat_waits_for_sensor_timeout_and_resets_on_valid_reading() {
    ...
    FakeSwitch blockedSwitch(makeSwitchConfig());
