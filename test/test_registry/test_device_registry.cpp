...
BoundedBlob<kMaxDeviceConfigBytes> encodeDummyConfig(const DummyDeviceConfigV1& config) {
    ...
    TEST_ASSERT_TRUE(ewfm::encodeFixedConfigBlob(DeviceBaseConfigV1::kMagic, config, buffer, dummyDeviceConfigSize(config)));
...
DeviceMutationResult renameDevice(DeviceRegistry& registry, DeviceId deviceId, const std::string& name, uint32_t now,
    ...
    TEST_ASSERT_TRUE(writeDeviceBaseConfig(blob, base));
    return registry.updateConfigAndDeps(deviceId, blob, 0, base, false, {}, 0, now, policy);
}
...
DeviceTypeDescriptor makeCommandableDescriptor(DeviceTypeId typeId, DeviceRole providedRole = DeviceRole::Unknown) {
    ...
    descriptor.ticks100ms = false;
    descriptor.ticks1s = false;
    descriptor.providedRoles = ProvidedRoles::of({providedRole});
...
DeviceTypeRegistry makeRegistryWithDummyProvidingOneWireBus() {
    ...
    dummyProvider.providedRoles = ProvidedRoles::of({DeviceRole::OneWireBus});
...
void assertDummyConfigName(const DeviceConfigBlobMap& configBlobs, DeviceId deviceId, const char* expected) {
    ...
    DummyDeviceConfigV1 config{};
    TEST_ASSERT_TRUE(decodeValidatedFixedConfigBlob(DeviceBaseConfigV1::kMagic, it->second.data(), it->second.size(), config));
...
void test_registry_update_config_ignores_reordered_unchanged_dependencies() {
    ...
    DeviceMutationResult updated =
        registry.updateConfigAndDeps(target.deviceId, encodeDummyConfig(config), DummyDevice::descriptor().currentConfigVersion, config,
                                     true, reorderedDeps, 2, 40, DevicePersistencePolicy::Immediate);
    TEST_ASSERT_TRUE_MESSAGE(updated.ok(), updated.validation.message);
...
void test_registry_update_config_and_deps_auto_escalates_real_reassignment_to_immediate() {
    ...
    DeviceMutationResult updated = registry.updateConfigAndDeps(target.deviceId, encodeDummyConfig(config), 1, config, true, reassignedDeps,
                                                                1, 40, DevicePersistencePolicy::Delayed);
    TEST_ASSERT_TRUE_MESSAGE(updated.ok(), updated.validation.message);
