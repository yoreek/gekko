...
struct TestRetainedStateRecord {
...
void test_device_registry_store_round_trip() {
    ...
    TEST_ASSERT_TRUE(decodeValidatedFixedConfigBlob(DeviceBaseConfigV1::kMagic, loadedConfigBlobs[1].data(), loadedConfigBlobs[1].size(),
                                                    loadedDependencyConfig));
    TEST_ASSERT_TRUE(decodeValidatedFixedConfigBlob(DeviceBaseConfigV1::kMagic, loadedConfigBlobs[2].data(), loadedConfigBlobs[2].size(),
                                                    loadedDependentConfig));
...
void test_device_retained_data_store_round_trip_and_remove() {
    ...
    TestRetainedStateRecord record{};
    record.deviceId = 7;
    record.outputState = 1U;
    TEST_ASSERT_TRUE(store.save(record).ok());

    TestRetainedStateRecord loaded;
    ...
    TestRetainedStateRecord missing;
...
void test_device_scoped_data_store_round_trip_and_clear() {
    ...
    TestRetainedStateRecord retained{};
    retained.deviceId = 42;
    retained.outputState = 1U;
    ...
    TEST_ASSERT_EQUAL_UINT8(1U, loaded.outputState);
...
void test_device_scoped_data_store_caches_missing_namespace_to_avoid_repeat_nvs_opens() {
    ...
    TestRetainedStateRecord reloaded{};
