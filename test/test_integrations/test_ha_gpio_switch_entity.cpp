...
SwitchOutputHaEntityAdapter makeGpioSwitchAdapter() {
    return SwitchOutputHaEntityAdapter({GpioSwitchDevice::descriptor().typeId, "gpio_switch", "mdi:toggle-switch"});
}
...
void test_ha_entity_adapter_registry_resolves_gpio_switch_and_rejects_unknown() {
    ...
    const IHaEntityAdapter* expanderAdapter = registry.find(PortExpanderSwitchDevice::descriptor().typeId);
    TEST_ASSERT_NOT_NULL(expanderAdapter);
    TEST_ASSERT_EQUAL_STRING("port_expander_switch", expanderAdapter->typeName());
    TEST_ASSERT_EQUAL_STRING("switch", expanderAdapter->haComponent());
...
void test_gpio_switch_ha_entity_adapter_builds_discovery_payload() {
    ...
    const SwitchOutputHaEntityAdapter adapter = makeGpioSwitchAdapter();
    ...
    TEST_ASSERT_EQUAL_STRING("Pump", output[ha::key::kName].as<const char*>());
    ...
    TEST_ASSERT_EQUAL_STRING("mdi:toggle-switch", output[ha::key::kIcon].as<const char*>());
}
...
void test_gpio_switch_ha_entity_adapter_builds_state_payload_for_on_off() {
    ...
    DeviceMutationResult on = registry.command(DeviceCommand{DeviceCommandType::SetOutput, created.deviceId, "true"}, 20);
...
void test_gpio_switch_ha_entity_adapter_applies_on_off_commands_case_insensitively() {
    ...
    TEST_ASSERT_TRUE(registry.runtime(created.deviceId)->switchOutputRuntime()->currentOutputState() == kSwitchOutputOn);

    TEST_ASSERT_TRUE(adapter.applyCommand(registry, *registry.runtime(created.deviceId), created.deviceId, "switch", "off", 30));
    TEST_ASSERT_TRUE(registry.runtime(created.deviceId)->switchOutputRuntime()->currentOutputState() == kSwitchOutputOff);
