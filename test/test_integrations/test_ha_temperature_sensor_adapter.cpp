...
// Minimal fake covering only what the temperature HA adapter needs to exercise -
...
void test_temperature_sensor_ha_entity_adapter_builds_discovery_payload() {
    ...
    TEST_ASSERT_EQUAL_STRING("Aquarium", output[ha::key::kName].as<const char*>());
    TEST_ASSERT_EQUAL_STRING(stateTopic.c_str(), output[ha::key::kStateTopic].as<const char*>());
    TEST_ASSERT_EQUAL_STRING("temperature", output[ha::key::kDeviceClass].as<const char*>());
    TEST_ASSERT_EQUAL_STRING("measurement", output[ha::key::kStateClass].as<const char*>());
    TEST_ASSERT_EQUAL_STRING("°C", output[ha::key::kUnitOfMeasurement].as<const char*>());
    TEST_ASSERT_EQUAL_STRING("mdi:thermometer", output[ha::key::kIcon].as<const char*>());
