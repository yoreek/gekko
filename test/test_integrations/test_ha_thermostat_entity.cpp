...
public:
    ...
    bool requestOutputState(OutputState state, uint32_t) override {
        state_ = state;
        return true;
    }
...
private:
    ...
    OutputState state_{OutputState::Off};
};
...
void bindThermostatIdentity(ThermostatDevice& device, DeviceId thermostatId, DeviceId temperatureSensorId, DeviceId switchId) {
    ...
    TEST_ASSERT_TRUE(encodeThermostatDeviceConfig(device.config(), buffer, size));
...
void test_thermostat_ha_entity_adapter_builds_discovery_payload() {
    ...
    TEST_ASSERT_EQUAL_STRING("Tank", output["name"].as<const char*>());
    TEST_ASSERT_EQUAL_STRING("mdi:thermostat", output["icon"].as<const char*>());

    JsonArray modes = output["modes"].as<JsonArray>();
    ...
    TEST_ASSERT_EQUAL_STRING("node1/climate_mode/42/state", output["mode_state_topic"].as<const char*>());
    TEST_ASSERT_EQUAL_STRING("node1/climate_mode/42/set", output["mode_command_topic"].as<const char*>());
    TEST_ASSERT_EQUAL_STRING("node1/climate_temperature/42/state", output["temperature_state_topic"].as<const char*>());
    TEST_ASSERT_EQUAL_STRING("node1/climate_temperature/42/set", output["temperature_command_topic"].as<const char*>());
    TEST_ASSERT_EQUAL_STRING("node1/climate_current_temperature/42/state", output["current_temperature_topic"].as<const char*>());
    TEST_ASSERT_EQUAL_STRING("node1/climate_action/42/state", output["action_topic"].as<const char*>());

    TEST_ASSERT_EQUAL_STRING("C", output["temperature_unit"].as<const char*>());
    TEST_ASSERT_EQUAL_FLOAT(0.0F, output["min_temp"].as<float>());
...
void test_thermostat_ha_entity_adapter_publishes_mode_temperature_and_action() {
    ...
    TEST_ASSERT_TRUE(thermostat.actualOutputState() == OutputState::On);
    ...
    TEST_ASSERT_TRUE(thermostat.actualOutputState() == OutputState::Off);
