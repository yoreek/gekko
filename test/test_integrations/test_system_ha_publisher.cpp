...
void test_system_ha_publisher_publishes_discovery_and_state_on_connect() {
    Fixture fx;
    fx.systemStats.set(123456, 12, 98304, 45056);
    fx.driver.rssiValue = -55;
    fx.driver.ssidValue = "office";
    fx.connectMqtt();

    TEST_ASSERT_TRUE(anyDiscoveryContains(fx.mqtt, "homeassistant/sensor/node1/node1_system_uptime/config", "\"dev_cla\":\"duration\""));
    TEST_ASSERT_TRUE(
        anyDiscoveryContains(fx.mqtt, "homeassistant/sensor/node1/node1_system_uptime/config", "\"stat_t\":\"~/system/uptime/state\""));
    TEST_ASSERT_TRUE(
        anyDiscoveryContains(fx.mqtt, "homeassistant/sensor/node1/node1_system_wifi_rssi/config", "\"dev_cla\":\"signal_strength\""));
    TEST_ASSERT_TRUE(anyDiscoveryContains(fx.mqtt, "homeassistant/button/node1/node1_system_restart/config", "\"dev_cla\":\"restart\""));
    TEST_ASSERT_TRUE(
        anyDiscoveryContains(fx.mqtt, "homeassistant/button/node1/node1_system_restart/config", "\"cmd_t\":\"~/system/restart/set\""));
    TEST_ASSERT_TRUE(anyDiscoveryContains(fx.mqtt, "homeassistant/sensor/node1/node1_system_uptime/config", "\"~\":\"node1\""));
    ...
    TEST_ASSERT_EQUAL_STRING("98304", publishedPayloadFor(fx.mqtt, "node1/system/min_free_heap/state").c_str());
    TEST_ASSERT_EQUAL_STRING("45056", publishedPayloadFor(fx.mqtt, "node1/system/largest_free_block/state").c_str());
    TEST_ASSERT_TRUE(anyDiscoveryContains(fx.mqtt, "homeassistant/sensor/node1/node1_system_largest_free_block/config",
                                          "\"stat_t\":\"~/system/largest_free_block/state\""));
...
void test_system_ha_publisher_restart_command_flushes_pending_persistence() {
    ...
    DeviceBaseConfigV1 renamedBase{};
    TEST_ASSERT_TRUE(assignDeviceBaseConfig(renamedBase, "dummy-renamed", true));
    DeviceMutationResult renamed = fx.registry.updateConfigAndDeps(created.deviceId, encodeDummyPayload("dummy-renamed"), 0, renamedBase,
                                                                   false, noDeps, 0, 20, DevicePersistencePolicy::Delayed);
