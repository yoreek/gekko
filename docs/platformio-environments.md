...
- `env:esp32dev_no_ble` is the production variant without BLE WiFi
  provisioning or its activation-button GPIO reservation.
...
- `esp32dev_no_ble` inherits the production configuration and removes
  `WITH_BLE_PROVISIONING`, `BLE_PROVISIONING_BUTTON_PIN`, and the `SimpleBLE`
  dependency.
- The no-BLE environment is compiled and packaged by CI, not by the local
...
- It makes the memory-saving no-BLE binary available to users without doubling
  every local commit build.
